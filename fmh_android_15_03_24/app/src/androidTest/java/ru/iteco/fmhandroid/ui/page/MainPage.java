...
public class MainPage {
    ...
    private final ViewInteraction menuMainButon = onView(allOf(withId(android.R.id.title), withText("Main"), isDisplayed()));
    ...
    public void mainPageMenuMainButton() {
        ...
        menuMainButon.check(matches(isDisplayed()));
        menuMainButon.check(matches(not(isClickable())));
    }
    public NewsPage mainPageMenuNewsButon() {
        appBarPage.menuNewsButon();
        return new NewsPage();
    }
    public AboutPage mainPageMenuAboutButon() {
        appBarPage.menuAboutButon();
